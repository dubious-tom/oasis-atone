...
func (suite *MiddlewareTestSuite) SetDenomRestrictions(addr sdk.AccAddress, denom, allowedChannel string) {
	...
	_, err := suite.chainA.ExecuteContract(addr, osmosisApp.AccountKeeper.GetModuleAddress(govtypes.ModuleName), []byte(denomRestriction), sdk.Coins{})
	suite.Require().NoError(err)
}
...
func (suite *MiddlewareTestSuite) fullSendTest(native bool) map[string]string {
	...
	channel := "channel-0"
	if !native {
		// TODO ATOMONE: IBC v10 renamed ParseDenomTrace to ExtractDenomFromPath, GetFullDenomPath to Path
...
func (suite *MiddlewareTestSuite) TestNonICS20() {
	...
	data := []byte("{}")
	// TODO ATOMONE: IBC v10 removed capability parameter from SendPacket
